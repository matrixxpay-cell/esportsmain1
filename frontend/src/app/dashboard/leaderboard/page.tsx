  ...
  return (
    <div className="pb-20 lg:pb-0">
      ...
      {!loading && top3.length >= 2 && (
        <div className="flex items-end justify-center gap-1.5 sm:gap-4 mb-8 mx-auto" style={{ maxWidth: '340px' }}>
          {(top3.length >= 3 ? [top3[1], top3[0], top3[2]] : [top3[1] || top3[0], top3[0], null].filter(Boolean)).map((player, i) => {
            const pos = top3.length >= 3 ? i : (i === 0 ? 1 : 0)
            const heights = ['h-[104px] sm:h-32', 'h-[130px] sm:h-40', 'h-[88px] sm:h-28']
            const badges = ['🥈', '🥇', '🥉']
            const glows = [
            ...
            const idx = top3.length >= 3 ? i : pos
            return (
              <motion.div key={player!.username || i}
                initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.1 + i * 0.1 }}
                className={`rounded-t-2xl ${heights[idx]} flex-1 max-w-[110px] sm:max-w-[130px] flex flex-col items-center justify-end pb-2.5 sm:pb-4 relative`}
                style={{
                  background: glows[idx],
                  border: `1px solid ${borderColors[idx]}`,
                  borderBottom: 'none',
                }}>
                <span className="text-xl sm:text-3xl mb-0.5 sm:mb-2">{badges[idx]}</span>
                <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl flex items-center justify-center text-[11px] sm:text-sm font-bold text-white mb-1 sm:mb-2 shadow-lg"
                  style={{ background: 'linear-gradient(135deg, #FF6B2B, #F59E0B)' }}>
                  {player!.username?.[0]?.toUpperCase() ?? '?'}
                </div>
                <div className="font-gaming font-bold text-[10px] sm:text-xs text-white text-center truncate px-1 w-full">
                  {player!.username}
                </div>
                <div className="text-slate-500 text-[9px] sm:text-xs mt-0.5 font-gaming">
      ...
      <div className="stat-card-v2 !p-0 overflow-hidden">
        <div className="grid grid-cols-[1.5rem_1fr_3.5rem] sm:grid-cols-[2rem_1fr_5rem_5rem] md:grid-cols-[2rem_1fr_5rem_5rem_4rem_5rem] px-3 sm:px-5 py-3 text-[11px] text-slate-500 uppercase tracking-wider border-b border-white/[0.06] font-medium gap-x-2">
          <div>#</div>
          <div>Player</div>
        ...
        {loading ? (
          <div className="space-y-0">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="grid grid-cols-[1.5rem_1fr_3.5rem] sm:grid-cols-[2rem_1fr_5rem_5rem] md:grid-cols-[2rem_1fr_5rem_5rem_4rem_5rem] px-3 sm:px-5 py-4 items-center border-b border-white/[0.03] gap-x-2">
          ...
          rest.map((player, i) => (
            <motion.div key={player.userId || player.username || i}
              ...
              className="grid grid-cols-[1.5rem_1fr_3.5rem] sm:grid-cols-[2rem_1fr_5rem_5rem] md:grid-cols-[2rem_1fr_5rem_5rem_4rem_5rem] px-3 sm:px-5 py-3 items-center border-b border-white/[0.03] hover:bg-white/[0.015] transition-colors gap-x-2">
              <div>
